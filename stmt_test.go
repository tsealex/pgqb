...
func TestSelect(t *testing.T) {
	...
	sql := strings.Trim(NewContext().Select(c1, c2).ToSQL(), " ")
	assert.Equal(t, `SELECT "school"."name", "school"."city" FROM "public"."school"`, sql)

	t2 := Table("public", "city")
	e1 := Column(t2, "name")
	e2 := Column(t2, "state")
	sql = strings.Trim(NewContext().Select(c1, e1.As("city"), e2).ToSQL(), " ")
